...
from utils.feature_shap import build_shap_background, compute_shap_contributions
...
def get_artifacts():
    ...
    plus the latent-vector pool, a fitted latent-space kNN index, and a
    k-means-summarized background sample (from the same pool) that the SHAP
    explainer reuses for every prediction in this session."""
    ...
    shap_background = build_shap_background(preprocessor, patient_profiles_pool)
    return (
        preprocessor,
        label_encoder,
        config,
...
if submitted:
    ...
    # --- SHAP: which features drove THIS prediction -- visible to both ---
    st.divider()
    st.subheader("Why this prediction?")
    st.caption(
        f"Top features pushing the model toward **{pred_label.replace('_', ' ')}**, "
        f"estimated with SHAP against a background sample from the train+val pool."
    )
    ...
    with st.spinner("Computing feature contributions..."):
        shap_df = compute_shap_contributions(
            ...
            device=device,
            background_summary=shap_background,
            preprocessor=preprocessor,
            sample_patient=input_df,
        )

    shap_display = shap_df.rename(
        columns={
            "feature": "Feature",
    ...
    st.bar_chart(shap_display.set_index("Feature")["Contribution"])
    st.dataframe(
        shap_display.style.format({"Contribution": "{:+.3f}"}),
        use_container_width=True,
        hide_index=True,
    )
